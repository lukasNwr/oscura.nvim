...
---setup oscura colorscheme
---@param configs OscuraConfig?
local function setup(configs)
   if type(configs) == "table" then
      user_configs = configs --[[@as OscuraConfig]]
   end
end

---load oscura colorscheme
local function load()
   if vim.fn.has("nvim-0.7") ~= 1 then
      vim.notify("oscura.nvim: you must use neovim 0.7 or higher")
      return
   end
